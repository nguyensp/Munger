...
class BaseMetricManager<T: MetricYearProtocol>: ObservableObject {
    ...
    // Safely calculate CAGR (Compound Annual Growth Rate)
    func calculateCAGR(startValue: Double, endValue: Double, years: Int) -> Double? {
        guard startValue != 0, years > 0 else { return nil }
        return pow(endValue / startValue, 1.0 / Double(years)) - 1
    }
}
